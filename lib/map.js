...
function initMap(data,cb) {
    ...
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(function (position) {
            ...
            map.setCenter(pos);
            map.setZoom(14);
            refreshMarker(map,marker,infowindow,infoWindowMyPos,data,pos,cb, tmp);
    ...
    autocomplete.addListener('place_changed', function() {
        ...
        // myPos[1].setPosition(place.geometry.location);

        tmp.setPosition(place.geometry.location);
        myPos[0].setAnimation(google.maps.Animation.BOUNCE);
        // myPos[1].setAnimation(google.maps.Animation.BOUNCE);
        setTimeout(function(){
            myPos[0].setAnimation(-1);
        },3000);
    ...
    function geocodeAddress(geocoder, resultsMap,adresse) {
        geocoder.geocode({'address': adresse}, function (results, status) {
            if (status === 'OK') {
                ...
                // myPos[1].setPosition(results[0].geometry.location);
                myPos[0].setAnimation(google.maps.Animation.BOUNCE);
                // myPos[1].setAnimation(google.maps.Animation.BOUNCE);
                setTimeout(function(){
                    myPos[0].setAnimation(-1);
                ...
                myAddress.coord=pos;
                input.value = adresse;
                refreshMarker(resultsMap,marker,infowindow,infoWindowMyPos,data,pos,cb);
            } else {
    ...
    var getMarker = function(){
        console.log(marker[0].getAnimation())
        return marker[0].getAnimation()
    }
