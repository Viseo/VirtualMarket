...
function initMap(data) {
    ...
    document.getElementById("divMap").style.zIndex='2';
    ...
    function research(research){
        ...
        setTimeout(function(){
            ...
            console.log(results)
            let adresse = results[0].getElementsByClassName('pac-item-query')[0].outerText + " " +results[0].children[2].textContent;
            console.log(adresse, "salut");
...
function refreshMarker(map,markers,infowindow,infowindowmypos,data,pos){
    ...
    if(tab.length==0){
        ...
        for(let i=0;i<point.length;i++){
            ...
            markers[i].addListener('click',function(){
                for(let mark in markers){
                    ...
                    console.log(markers[mark].getAnimation())
                }
                let distance=distanceGPS({lat:markers[i].position.lat(),lng:markers[i].position.lng()},myAddress.coord);
                console.log({lat:markers[i].position.lat(),lng:markers[i].position.lng()},myAddress.coord)
                infowindow.setContent('Ce point relai est à : '+distance.toFixed(2)+" km"+"<div><strong>"+markers[i].title+"</strong>");
        ...
        for(let mark in markers){
            console.log(markers[mark].title,nearest.ob.address)
            if(markers[mark].title==nearest.ob.address){
                markers[mark].setIcon(green);
                markers[mark].setAnimation(google.maps.Animation.BOUNCE);
                infowindow.setContent('Ce point relai est à : '+nearest.ob.dist.toFixed(2)+" km"+"<div><strong>"+markers[mark].title+"</strong>");
...
function aroundAddress(ref,data){
    ...
    for (let point in data){
        ...
        if (data[point].dist <= 1){
            around.push(data[point]);
        }
        if (data[point].dist <= 20){
            nothing=false;
        }
    }
