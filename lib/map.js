    ...
    geocoder.geocode({'location': latlng}, function(results, status) {
        if (status === 'OK') {
            if (results[1]) {
                ...
                (typeof latlng.lat=="function"&& typeof latlng.lng=="function")?myAddress.coord={lat: latlng.lat(),lng: latlng.lng()}:{}
    ...
    if(tab.length==0){
        ...
        for(let mark in markers){
            if(markers[mark].title==nearest.ob.address){
                ...
                infowindow.setContent(("<div><strong>"+markers[mark].title+"</strong></br>"+"Ce point relai est à : "+nearest.ob.dist.toFixed(2)+" km"));
