...
exports.SVG = function(runtime) {
    ...
    class Translation extends Handler {
        ...
        shadow(name,dx,dy,blur){
            this.filter= svgr.create("filter");
            svgr.attr(this.filter, "id", ""+name);
            svgr.attr(this.filter, "width", "150%");
            svgr.attr(this.filter, "height", "150%");

            this.offset= svgr.create("feOffset");
            svgr.attr(this.offset, "result", "offOut");
            svgr.attr(this.offset, "in", "SourceGraphic");
            svgr.attr(this.offset, "dx", ""+dx);
            svgr.attr(this.offset, "dy", ""+dy);

            this.matrix= svgr.create("feColorMatrix");
            svgr.attr(this.matrix, "result", "matrixOut");
            svgr.attr(this.matrix, "in", "offOut");
            svgr.attr(this.matrix, "type", "matrix");
            svgr.attr(this.matrix, "values", "0.2 0 0 0 0 0 0.2 0 0 0 0 0 0.2 0 0 0 0 0 1 0");

            this.gaussian= svgr.create("feGaussianBlur");
            svgr.attr(this.gaussian, "result", "blurOut");
            svgr.attr(this.gaussian, "in", "matrixOut");
            svgr.attr(this.gaussian, "stdDeviation", ""+blur);

            this.blend= svgr.create("feBlend");
            svgr.attr(this.blend, "in", "SourceGraphic");
            svgr.attr(this.blend, "in2", "blurOut");
            svgr.attr(this.blend, "mode", "normal");

            svgr.add(this.filter,this.offset);
            svgr.add(this.filter,this.matrix);
            svgr.add(this.filter,this.gaussian);
            svgr.add(this.filter,this.blend);
            svgr.attr(this.component, "filter", "url(#"+name+")");
            svgr.add(this.component,this.filter)
            return this;
        }
    }
    ...
    class Path extends Shape {
        ...
        arc(rx, ry, ax, lf, sf, x1, y1) {
            this.drawing += "A " + rx + " " + ry + " " + ax + " " + lf + " " + sf + " " + x1 + " " + y1 + " ";
            this.points.push({x: x1, y: y1, type:"arc-end"});
            this._draw();
            this.reshapeHandler && this.reshapeHandler(this.points);
            return this;
        }
    ...
    return {
        ...
        GREY_GREEN : [133, 193, 126],
        // GREY_GREEN : [104, 157, 113],
